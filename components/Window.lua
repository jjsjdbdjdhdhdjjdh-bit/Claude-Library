...
function Window:CreateHomeTab(config)
    ...
    task.spawn(function()
        ...
        local success, result = pcall(function() return plr:GetFriendsOnline() end)
    ...
    clickArea.MouseButton1Click:Connect(function() self:SelectTab(TAB_TITLE) end)
end
...
function Window:CreateHomeTab(config)
    ...
    --   CHANGELOG SYSTEM
    -- ════════════════════════════════════════════════════════
    -- This section creates the changelog panel.
    -- It accepts a list of entries in config.Changelog.
    -- Each entry should be a table: { Date = "YYYY-MM-DD", Title = "Title", Desc = "Description" }
    ...
    -- Iterate through logs and create UI entries
    for i, log in ipairs(logs) do
        ...
        -- Entry Content (Rich Text supported)
        inst("TextLabel", {
            Size            = UDim2.new(1, -12, 0, 0),
            AutomaticSize   = Enum.AutomaticSize.Y,
            Position        = UDim2.new(0, 12, 0, 0),
            BackgroundTransparency = 1,
            -- Format: Colored Title - Date \n Description
